...
{// const grasp_estimator::DataAcquired::ConstPtr msg_data_acquired_
 ...
 if( (this->msg_data_acquired_now.hand_index_distal_link.transform.translation.x!=0) &&
  ...
  marker_sph.color.r = 1.0-(float)(pow(chi, 2.0) / dof);
  marker_sph.color.g = (float)(pow(chi, 2.0) / dof);
  ...
  marker_cyl.color.r = (float)(pow(chi_cyl, 2.0) / dof_cyl);
  marker_cyl.color.g = 0.0f;
  marker_cyl.color.b = 1.0f-(float)(pow(chi_cyl, 2.0) / dof_cyl);
  ...
  gsl_vector_view w_ell = gsl_vector_view_array(weights, n);
  ...
  marker_ell.color.g = 1.0f-(float)(pow(chi_ell, 2.0)/dof_ell);
  marker_ell.color.b = (float)(pow(chi_ell, 2.0)/dof_ell);
  ...
  gsl_vector_view w_plan = gsl_vector_view_array(weights, n);
  ...
  //ROS_INFO( "Index_plan = %g",  index_plan);
   ...
   // ROS_INFO ( "a    = %.5f +/- %.5f\n", FIT_plan(0), c_plan*ERR_plan(0));
   // ROS_INFO ( "b    = %.5f +/- %.5f\n", FIT_plan(1), c_plan*ERR_plan(1));
   // ROS_INFO ( "c    = %.5f +/- %.5f\n", FIT_plan(2), c_plan*ERR_plan(2));
   // ROS_INFO ( "d    = %.5f +/- %.5f\n", FIT_plan(3), c_plan*ERR_plan(3));
  ...
  //marker_box.pose.position.x = center_x;
  //marker_box.pose.position.y = center_y;
  //marker_box.pose.position.z = center_z;
  ...
  marker_box.color.r = 1.0f-float(pow(chi_plan, 2.0) / dof_plan);
  marker_box.color.g = 1.0f-float(pow(chi_plan, 2.0) / dof_plan);
  marker_box.color.b = (float)(pow(chi_plan, 2.0) / dof_plan);
  ...
  // if ((r_cylinder<0.01||r_cylinder>0.04 || (1.0 - ratio_inv_cyl) < 0.75) && (r_sphere<0.04 || r_sphere>0.089 || (1.0 - ratio_inv) < 0.75))
  // {
