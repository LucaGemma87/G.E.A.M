  ...
  public:
      ...
      int count_bad_rec,count_good_rec;
...
ROS_INFO("Load Database and Index from disk");  
  ...
  ROS_INFO("5) Flann ChiSquareDistance = %g",flann_dists[0][0]);
   ...
   if (rec==0) 
   { 
     this->count_bad_rec=this->count_bad_rec+1;
     this->count_good_rec=this->count_good_rec;
   }
   else  {
     this->count_good_rec=this->count_good_rec+1;  
     this->count_bad_rec=this->count_bad_rec;
  ...
  { 
    ...
    int num_test(0);
    double result_bad,result_good;
    ros::spinOnce();
     while (ros::ok()&& object_clustering_recognition_server.num_object_test<object_clustering_recognition_server.count_centroid)
     {
       ...
       sleep(1.0);
      result_bad=(object_clustering_recognition_server.count_bad_rec/num_test);
    
      result_good=(object_clustering_recognition_server.count_good_rec/num_test); 
    
     ROS_INFO("Good recognition = %5.2f ",(float)result_good);
     ROS_INFO("Bad recognition = %5.2f ",(float)result_bad);
     sleep(1.0);
