  ...
  private:
	...
	std::vector<std::vector<int> > exp_grasp_type_object;
     ...
     std::string test_object_string_now;
  ...
  public:
      ...
      std::vector<int> dlib_result_single_object;
      std::vector<int> flann_result_single_object;
      int test_sample;
      // FLANN
      ...
      double dlib_count_bad_rec,dlib_count_good_rec;
      double flann_count_bad_rec,flann_count_good_rec;
	...
	dlib_count_bad_rec =0;
	dlib_count_good_rec =0;
        flann_count_bad_rec =0;
	flann_count_good_rec =0;
  	}  
      	//! Empty stub
  ...
  //   std_srvs::Empty::Request req_;
   ...
   void Object_Clustering_Recognition_Server::offline_add_data()
  {
   //m.clear();
   ...
   flann_query.reset();
    ...
    ROS_INFO_STREAM(" <----------- NEW TEST ------------------>");   
    ROS_INFO_STREAM("Test data completed for "+object_recognition[test_sample]);
    test_object_string_now=object_recognition[test_sample];
  ...
  //        'left_hand_ring_outer_joint_mimic', 'left_hand_synergy_joint', 'left_hand_thumb_abd_joint', 'left_hand_thumb_inner_joint', 'left_hand_thumb_inner_joint_mimic', 'left_hand_thumb_outer_joint', 'left_hand_thumb_outer_joint_mimic']
       ...
       // index 
       m(28) = object_data.hand_joint_curr_state.position[5]; //  'left_hand_little_abd_joint'
       m(29) = object_data.hand_joint_curr_state.position[6]; //'left_hand_index_outer_joint'
       ...
       m(32) =  object_data.hand_joint_curr_state.position[12];// 'left_hand_middle_abd_joint'
       m(33) =  object_data.hand_joint_curr_state.position[13];  // 'left_hand_little_outer_joint'
       ...
       m(36) =  object_data.hand_joint_curr_state.position[19]; // 'left_hand_ring_abd_joint'
       m(37) =  object_data.hand_joint_curr_state.position[20]; // 'left_hand_middle_outer_joint'
       ...
       m(40) =  object_data.hand_joint_curr_state.position[26]; //  'left_hand_thumb_abd_joint'
       m(41) =  object_data.hand_joint_curr_state.position[27]; // 'left_hand_ring_outer_joint'
...
//       flann_object_recognition.resize(num_dataset);
	   ...
	   std::string j_string;  std::ostringstream convert;
		...
		{  
                 ...
                 for ( int ii =0; ii<data_in.size(); ++ii)
		 {
	           ...
	           if(index_database==(num_dataset-12))
                  {
 	...
 	for (int j = 0; j < object_type_grasp_database[i]; ++j)
       ...
       test_samples.resize(count_centroid);
       dlib_result_single_object.resize(count_centroid);
       flann_result_single_object.resize(count_centroid);
       flann_database.reset();
       flann_query.reset();
...
//    // 3 element from Imu Sensor
  ...
  rbf_trainer.set_kernel(kernel_type(0.05));
...
ROS_INFO("Load Database and Index from disk");  
  ...
  ROS_INFO("<---------- Multi Training on database ---------->");  
   ...
   krr_trainer<kernel_type> rbf_trainer;
   //ROS_INFO("<---------- 2---------->");
   rbf_trainer.set_kernel(kernel_type(0.005));
   //ROS_INFO("<---------- 3 ---------->");
   trainer.set_trainer(rbf_trainer);
  // //randomize_samples(samples, labels);
  //
   ...
   if (object_recognition[index_ob]!=test_object_string_now) 
   { 
     ROS_INFO("Dlib bad recognition");
     this->dlib_count_bad_rec=this->dlib_count_bad_rec+(int)1;
     this->dlib_count_good_rec=this->dlib_count_good_rec;
     
   }
   else  {
     ROS_INFO("Dlib good recognition");
     this->dlib_count_good_rec=this->dlib_count_good_rec+(int)1;  
     this->dlib_count_bad_rec=this->dlib_count_bad_rec;
     dlib_result_single_object[test_sample]=dlib_result_single_object[test_sample]+1;
  } 
  ...
  ROS_INFO("5) Flann ChiSquareDistance = %g",flann_dists[0][0]);
   ...
   if (flann_object_recognition[flann_indices[0][0]]!=test_object_string_now) 
   { 
     ROS_INFO("Flann bad recognition");
     this->flann_count_bad_rec=this->flann_count_bad_rec+(int)1;
     this->flann_count_good_rec=this->flann_count_good_rec;
   }
   else  {
     ROS_INFO("Flann good recognition");
     this->flann_count_good_rec=this->flann_count_good_rec+(int)1;  
     this->flann_count_bad_rec=this->flann_count_bad_rec;
     flann_result_single_object[test_sample]=flann_result_single_object[test_sample]+1;
  ...
  { 
    ...
    double num_test(0.0);
    double dlib_result_bad,dlib_result_good,flann_result_bad,flann_result_good;
    double dlib_result_perc_ob[11], flann_result_perc_ob[11];
    ros::spinOnce();
     while (ros::ok()&& object_clustering_recognition_server.num_object_test<10)
     {
       ROS_INFO("Testing %d",object_clustering_recognition_server.num_object_test);
       object_clustering_recognition_server.loadDataBase();
       //ROS_INFO("Test sampling size %d",(int)object_clustering_recognition_server.test_samples.size());
       //
       
       for(int i = 0; i<object_clustering_recognition_server.count_centroid;++i)
       { 
       object_clustering_recognition_server.loadDataBase();
       ROS_INFO("Test sampling size %d",(int)object_clustering_recognition_server.test_samples.size());
       object_clustering_recognition_server.clustering();   
       object_clustering_recognition_server.test_sample=i;
       ROS_INFO("Test sampling %d",(int)i);
       object_clustering_recognition_server.offline_add_data();
       object_clustering_recognition_server.testing(); // <---- to delete
       
       num_test+=1;
       ROS_INFO("test number = %d ",(int)num_test);
  
      dlib_result_bad=(object_clustering_recognition_server.dlib_count_bad_rec/num_test);
    
      dlib_result_good=(object_clustering_recognition_server.dlib_count_good_rec/num_test); 
     
      flann_result_bad=(object_clustering_recognition_server.flann_count_bad_rec/num_test);
    
      flann_result_good=(object_clustering_recognition_server.flann_count_good_rec/num_test); 
     
     
    
        dlib_result_perc_ob[i]=(float)object_clustering_recognition_server.dlib_result_single_object[i]/10.0;
      
        flann_result_perc_ob[i]=(float)object_clustering_recognition_server.flann_result_single_object[i]/10.0;
        /*
          ROS_INFO("Dlib Good recognition for object %d = %5.2f ",(int)i,(float)dlib_result_perc_ob[i]);
    
          ROS_INFO("Flann Good recognition for object %d = %5.2f ",(int)i,(float)flann_result_perc_ob[i]);
	...
	*/
       ...
       object_clustering_recognition_server.num_object_test+=1;
     }  
     ROS_INFO_STREAM("<------------------FINAL  RESULTS ------------------>");
     ROS_INFO("Dlib Good recognition = %5.2f ",(float)dlib_result_good);
     ROS_INFO("Dlib Bad recognition = %5.2f ",(float)dlib_result_bad);
     ROS_INFO("Flann Good recognition = %5.2f ",(float)flann_result_good);
     ROS_INFO("Flann Bad recognition = %5.2f ",(float)flann_result_bad);
