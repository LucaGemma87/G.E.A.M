...
{  
  ...
  std::string service_name("data_logger_srv");
  ...
  { 
    ...
    // check joint state
    ROS_INFO(" Data_Acquiring_srv.data.right_hand_joint_curr_state.position[0] %f ",(float)Data_Acquiring_srv.response.data.right_hand_joint_curr_state.position[0]);
    
    // check right hand index distal link
    tf::StampedTransform wrist_2_right_hand_index_distal_link_stamped;
    transformStampedMsgToTF(Data_Acquiring_srv.response.data.right_hand_index_distal_link,wrist_2_right_hand_index_distal_link_stamped);
    ...
    ROS_INFO("Motor right hand synergy position %f",(float)Data_Acquiring_srv.response.data.right_hand_synergy_joint_state_position);
    ROS_INFO("Motor right hand synergy velocity %f",(float)Data_Acquiring_srv.response.data.right_hand_synergy_joint_state_velocity);
    ROS_INFO("Motor right hand synergy effort %f",(float)Data_Acquiring_srv.response.data.right_hand_synergy_joint_state_effort);
